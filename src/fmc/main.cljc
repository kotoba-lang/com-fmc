(ns fmc.main
  "Kotodama WASM entrypoint for the Fmc clean-room actor (L4) — Clojure port.
  ...
  facts (`fmc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"fmc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fmc-compat" :tier tier :entities entities} 200])
